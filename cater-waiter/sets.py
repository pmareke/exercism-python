...
from sets_categories_data import (VEGAN, VEGETARIAN, KETO, PALEO, OMNIVORE,
...
def clean_ingredients(dish_name, dish_ingredients):
    ...
    This function should return a `tuple` with the name of the dish as the
    first item, followed by the de-duped `set` of ingredients as the second
    item.
    """
...
def check_drinks(drink_name, drink_ingredients):
    ...
    The function should return the name of the drink followed by "Mocktail" if
    the drink has
    no alcoholic ingredients, and drink name followed by "Cocktail" if the
    drink includes alcohol.
    """
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    ingredients = VEGAN, VEGETARIAN, PALEO, KETO, OMNIVORE
    message = ''
    for index, i in enumerate(ingredients, 0):
        if i.issuperset(dish_ingredients):
            message = f'{dish_name}: {(categories[index])}'
    return message
...
def compile_ingredients(dishes):
    ...
    This function should return a `set` of all ingredients from all listed
    dishes.
    """

    dish_set = set()
    for i in dishes:
        dish_set = dish_set.union(i)
    return dish_set
...
def singleton_ingredients(dishes, intersection):
    ...
    :param intersection: constant - one of (VEGAN_INTERSECTION,
    VEGETARIAN_INTERSECTION,PALEO_INTERSECTION, KETO_INTERSECTION,
    OMNIVORE_INTERSECTION)
    :param dishes:  list of ingredient sets
    :return: set of singleton ingredients

    Each dish is represented by a `set` of its ingredients.
    Each `<CATEGORY>_INTERSECTION` is an `intersection` of all dishes in the
    category.
    ...
    for i in dishes:
        dish_set = dish_set.union(i)
    return dish_set.difference(intersection)
